...
cv2.createTrackbar("Center Min X (%)", "Controls", 45, 100, nothing)
cv2.createTrackbar("Center Max X (%)", "Controls", 55, 100, nothing)
cv2.createTrackbar("Center Min Y (%)", "Controls", 45, 100, nothing)
cv2.createTrackbar("Center Max Y (%)", "Controls", 55, 100, nothing)

# Deques to store last N pupil positions for smoothing
N = 5
cx_history = deque(maxlen=N)
cy_history = deque(maxlen=N)
    ...
    for (x, y, w, h) in faces:
        ...
        for (ex, ey, ew, eh) in eyes:
            ...
            if contours:
                ...
                smooth_cx = sum(cx_history) / len(cx_history)
                smooth_cy = sum(cy_history) / len(cy_history)
                ...
                if rel_cx < center_min_x:
                    horiz = 'LEFT'
                elif rel_cx > center_max_x:
                    horiz = 'RIGHT'
                else:
                    horiz = 'CENTER'

                # Vertical direction
                if rel_cy < center_min_y:
                    vert = 'UP'
                elif rel_cy > center_max_y:
                    vert = 'DOWN'
                else:
                    vert = 'CENTER'
